...
async function withTargetLock(target, operation) {
  const key = path.resolve(target);
  const previous = targetLocks.get(key);
  let release;
...
async function fileMatches(target, expectedBytes, expectedSha256, signal) {
  let stat;
  try {
...
async function prepareVerifiedFile({
  ...
  return withTargetLock(target, async () => {
    ...
    try {
      ...
      if (ownsTemporary) await fs.rm(temporary, { force: true });
      throw error;
    }
  });
}
