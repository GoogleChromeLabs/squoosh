...
const include = (source, text, file) =>
  assert.ok(source.includes(text), `${file} must include ${JSON.stringify(text)}`);
const exclude = (source, text, file) =>
  assert.ok(!source.includes(text), `${file} must exclude ${JSON.stringify(text)}`);
...
include(toCache, "import * as pngDecoder from 'entry-data:codecs/png/pkg';", 'to-cache');
for (const role of [
  'features_worker',
  'model',
  'ort_asyncify_mjs',
...
include(toCache, 'new Set(ben2AssetInventory.map((asset) => asset.path)).size !== 6', 'to-cache');
include(toCache, "export const initial = ['/', '/manifest.json', ...initialJs];", 'to-cache');
include(toCache, 'return [...new Set(items)].filter((item) => !ben2Assets.includes(item));', 'to-cache');
