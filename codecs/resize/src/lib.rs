...
pub fn resize(
    ...
    if premultiply {
      for i in 0..num_input_pixels {
        for j in 0..3 {
          input_image[4 * i + j] = ((input_image[4 * i + j] as f32) * (input_image[4 * i + 3] as f32)/255.0) as u8;
        }
      }
    }
    ...
    if premultiply {
      for i in 0..num_output_pixels {
        for j in 0..3 {
          output_image[4 * i + j] = ((output_image[4 * i + j] as f32) * 255.0 / (output_image[4 * i + 3] as f32)) as u8;
        }
      }
    }
